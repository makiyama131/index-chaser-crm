                    ...
                    <form action="{{ route('customers.update', $customer) }}" method="POST">
                        ...
                        @method('PUT') <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name', $customer->name) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                            </div>
                            ...
                            <div>
                                ...
                                <select name="rank" id="rank" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                    <option value="A" {{ old('rank', $customer->rank) == 'A' ? 'selected' : '' }}>A: 今すぐ</option>
                                    <option value="B" {{ old('rank', $customer->rank) == 'B' ? 'selected' : '' }}>B: 検討中</option>
                                    <option value="C" {{ old('rank', $customer->rank) == 'C' ? 'selected' : '' }}>C: 情報収集</option>
                                </select>
                            </div>
                        </div>
